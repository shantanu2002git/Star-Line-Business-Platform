...
import { getOrder, listNotifications, listOrders, markNotificationRead, type Order } from '@/lib/ecommerce';
...
function NotificationCenter({ limit = 6 }: { limit?: number }) {
  const demoNotifications = [
    ...
    { id: 'n-2', title: 'Pickup ready', message: 'Order #SL-1842 is ready for collection after 16:30 today.', read: false },
    { id: 'n-3', title: 'Invoice sent', message: 'The invoice for your latest banner order is ready to review.', read: true },
    { id: 'n-4', title: 'New service added', message: 'We have added a custom design service for your next request.', read: true },
...
function OrderHistoryPage() {
  ...
  useEffect(() => { void listOrders('Ananya Shah').then(setOrders); }, []);
  const [, setLocation] = useLocation();
  return <Shell mode="portal"><PageHeading eyebrow="Customer desk / orders" title="Order history" body="Track every order and its latest status." />{orders.length ? <div className="grid gap-3">{orders.map((order) => <button key={order.id} onClick={() => setLocation(`/portal/order/${order.id}`)} className="rounded-2xl border border-border bg-card p-4 text-left transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex items-start justify-between gap-3"><div><div className="font-mono-ui text-xs font-bold">{order.number}</div><div className="mt-1 text-sm text-muted-foreground">{order.createdAt} · {order.items.length} item{order.items.length === 1 ? '' : 's'}</div></div><Status value={order.status} /></div><div className="mt-4 flex items-center justify-between text-sm"><span>{order.items.map((item) => item.name).join(', ')}</span><strong>{money(order.total)}</strong></div></button>)}</div> : <EmptyState title="No orders yet" body="Your completed purchases will appear here." action={<ActionButton onClick={() => setLocation('/portal/catalog')}>Shop now</ActionButton>} />}</Shell>;
}
...
function CustomerOverview() {
  ...
  const Card = ({ product, badge }: { product: any; badge: string }) => <button onClick={() => setLocation(`/portal/products/${product.id}`)} className="group min-w-[76vw] overflow-hidden rounded-2xl border border-border bg-card text-left transition hover:-translate-y-1 hover:shadow-lg sm:min-w-0"><div className="relative"><ProductImage product={product} /><span className="absolute left-3 top-3 rounded-full bg-accent px-2.5 py-1 text-[10px] font-bold uppercase text-accent-foreground">{badge}</span></div><div className="p-3"><div className="font-display text-base font-bold">{product.name}</div><div className="mt-1 text-sm font-bold">{money(product.price)}</div></div></button>;
  return <Shell mode="portal"><PageHeading eyebrow="Customer desk / overview" title="Good morning, Ananya." body="Find something useful, track your work, and keep moving." action={<ActionButton onClick={() => setLocation('/portal/catalog')}><ShoppingBag className="h-4 w-4" /> Shop now</ActionButton>} /><div className="rounded-2xl bg-primary p-5 text-primary-foreground shadow-lg"><div className="flex items-start justify-between gap-3"><div><div className="font-mono-ui text-[10px] uppercase tracking-[.16em] text-accent">Next pickup</div><h2 className="mt-2 font-display text-2xl font-bold">Order #SL-1842</h2><p className="mt-1 text-xs text-primary-foreground/60">Print & bind · Ready after 16:30</p></div><Status value="Ready today" /></div><button onClick={() => setLocation('/portal/requests')} className="mt-5 rounded-xl bg-accent px-4 py-2.5 text-xs font-bold text-accent-foreground">View details</button></div><section className="mt-6 rounded-2xl border border-border bg-card p-4 sm:p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="font-display text-lg font-bold">New items</h2><p className="mt-1 text-xs text-muted-foreground">Fresh picks for your shop.</p></div><button onClick={() => setLocation('/portal/catalog?new=1')} className="text-xs font-bold text-primary">View all</button></div>{products.isLoading ? <SkeletonRows count={2} /> : <div className="portal-scroll-row -mx-1 flex gap-3 overflow-x-auto px-1 pb-1 sm:grid sm:grid-cols-2 sm:overflow-visible">{allProducts.slice(0, 2).map((product) => <Card key={product.id} product={product} badge="New" />)}</div>}</section><section className="mt-4 rounded-2xl border border-border bg-card p-4 sm:p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="font-display text-lg font-bold">Popular products</h2><p className="mt-1 text-xs text-muted-foreground">Trending at Star Line.</p></div><button onClick={() => setLocation('/portal/catalog?popular=1')} className="text-xs font-bold text-primary">View all</button></div>{products.isLoading ? <SkeletonRows count={3} /> : <div className="portal-scroll-row -mx-1 flex gap-3 overflow-x-auto px-1 pb-1 sm:grid sm:grid-cols-3 sm:overflow-visible">{allProducts.filter((product) => product.featured).slice(0, 3).map((product) => <Card key={product.id} product={product} badge="Popular" />)}</div>}</section><div className="mt-4 flex flex-col gap-3 rounded-2xl border border-border bg-card p-4 sm:p-5"><div className="flex items-center justify-between"><div><h2 className="font-display text-lg font-bold">Service request</h2><p className="mt-1 text-xs text-muted-foreground">Need help with printing, design, or setup?</p></div><FileText className="h-5 w-5 text-muted-foreground" /></div><ActionButton onClick={() => setLocation('/portal/requests/new')} testId="button-home-service-request"><Plus className="h-4 w-4" /> Start a request</ActionButton></div></Shell>;
}
...
function PageHeading({ eyebrow, title, body, action }: { eyebrow: string; title: string; body?: string; action?: ReactNode }) {
  return <div className="mb-5 flex flex-col justify-between gap-3 sm:mb-7 sm:flex-row sm:items-end"><div><div className="mb-1.5 flex items-center gap-2 font-mono-ui text-[9px] font-medium uppercase tracking-[.16em] text-muted-foreground"><span className="h-1.5 w-1.5 rounded-full bg-accent" />{eyebrow}</div><h1 data-testid={`text-page-title-${title.toLowerCase().replaceAll(' ', '-')}`} className="font-display text-2xl font-bold tracking-[-.035em] sm:text-4xl">{title}</h1>{body && <p className="mt-1.5 max-w-2xl text-xs leading-5 text-muted-foreground sm:text-sm sm:leading-6">{body}</p>}</div>{action}</div>;
}
...
function CustomerOverviewLegacyUnused() {
  const [, setLocation] = useLocation();
  const products = useListProducts({ pageSize: 100 });
  const allProducts = products.data?.items || [];
  const newItems = allProducts.slice(0, 2);
  const popularProducts = allProducts.filter((product) => product.featured).slice(0, 3);
  const ProductCard = ({ product, badge }: { product: any; badge: string }) => <button onClick={() => setLocation(`/portal/products/${product.id}`)} className="group overflow-hidden rounded-2xl border border-border bg-card text-left transition hover:-translate-y-1 hover:border-primary/40 hover:shadow-lg"><div className="relative flex h-36 items-center justify-center bg-secondary"><ProductImage product={product} /><span className="absolute left-3 top-3 rounded-full bg-accent px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-accent-foreground">{badge}</span></div><div className="p-4"><div className="font-display text-base font-bold">{product.name}</div><div className="mt-2 text-sm font-bold">{money(product.price)}</div></div></button>;
  return <Shell mode="portal"><PageHeading eyebrow="Customer desk / overview" title="Good morning, Ananya." body="Your work, your orders, and the next useful thing — all in one clear line." action={<ActionButton onClick={() => setLocation('/portal/requests/new')} testId="button-new-request"><Plus className="h-4 w-4" /> New service request</ActionButton>} /><div className="grid gap-4 md:grid-cols-3"><div className="rounded-2xl bg-primary p-5 text-primary-foreground md:col-span-2"><div className="flex items-start justify-between"><div><p className="text-sm text-primary-foreground/60">Your next pickup</p><h2 className="mt-2 font-display text-3xl font-bold">Order #SL-1842</h2></div><Status value="Ready today" /></div><div className="mt-10 flex items-end justify-between"><div><p className="text-xs text-primary-foreground/60">Print & bind · 42 pages</p><p className="mt-1 text-sm font-semibold">Ready after 16:30</p></div><button onClick={() => setLocation('/portal/requests')} className="rounded-xl bg-accent px-4 py-2 text-xs font-bold text-accent-foreground">View details</button></div></div><div className="rounded-2xl border border-border bg-card p-5"><div className="flex items-center justify-between"><p className="text-sm font-bold">Need something?</p><Sparkles className="h-5 w-5 text-accent-foreground" /></div><p className="mt-3 text-sm leading-6 text-muted-foreground">Upload your documents and we will take it from there.</p><button onClick={() => setLocation('/portal/requests/new')} className="mt-8 inline-flex items-center gap-2 text-sm font-bold text-primary hover:gap-3">Start a request <ArrowRight className="h-4 w-4" /></button></div></div><section className="mt-8 rounded-2xl border border-border bg-card p-5 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><h2 className="font-display text-xl font-bold">New items</h2><p className="mt-1 text-sm text-muted-foreground">Freshly added products for your shop.</p></div><button onClick={() => setLocation('/portal/catalog?new=1')} className="text-xs font-bold text-primary hover:underline">View all</button></div>{products.isLoading ? <SkeletonRows count={2} /> : <div className="grid gap-3 sm:grid-cols-2">{newItems.map((product) => <ProductCard key={product.id} product={product} badge="New" />)}</div>}</section><section className="mt-6 rounded-2xl border border-border bg-card p-5 sm:p-6"><div className="mb-5"><h2 className="font-display text-xl font-bold">Popular products</h2><p className="mt-1 text-sm text-muted-foreground">Trending picks from the counter.</p></div>{products.isLoading ? <SkeletonRows count={3} /> : <div className="grid gap-3 sm:grid-cols-3">{popularProducts.map((product) => <ProductCard key={product.id} product={product} badge="Popular" />)}</div>}</section></Shell>;
}

const CustomerOverviewLegacy = CustomerOverview;

function CommerceCatalogPage() {
  const [, setLocation] = useLocation();
  ...
  const [search, setSearch] = useState('');
  const [category, setCategory] = useState('all');
  const newOnly = location.includes('new=1');
  const popularOnly = location.includes('popular=1');
  const items = (products.data?.items || []).filter((product) => product.name.toLowerCase().includes(search.toLowerCase()) && (category === 'all' || product.category === category) && (!newOnly || product.id === 'prod-101' || product.id === 'prod-102') && (!popularOnly || product.featured));
  return <Shell mode="portal"><PageHeading eyebrow="Customer desk / catalog" title={newOnly ? 'New items' : 'The useful things.'} body="Small essentials, practical upgrades, and everyday counter supplies." action={<Link href="/portal/requests/new" className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground">Request printing <ArrowRight className="h-4 w-4" /></Link>} /><div className="mb-6 flex flex-col gap-3 sm:flex-row"><SearchBar value={search} onChange={setSearch} placeholder="Search products..." /><div className="flex gap-2 overflow-x-auto">{['all', ...(categories.data || []).map((item) => item.name)].map((item) => <button key={item} onClick={() => setCategory(item)} className={cx('whitespace-nowrap rounded-xl border px-3 py-2 text-xs font-bold', category === item ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-card text-muted-foreground')}>{item === 'all' ? 'All items' : item}</button>)}</div></div>{products.isLoading ? <SkeletonRows count={6} /> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{items.map((product) => <button key={product.id} onClick={() => setLocation(`/portal/products/${product.id}`)} className="group overflow-hidden rounded-2xl border border-border bg-card text-left transition hover:-translate-y-1 hover:shadow-lg"><ProductImage product={product} /><div className="p-4"><div className="font-display text-lg font-bold">{product.name}</div><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{product.description}</p><div className="mt-4 font-bold">{money(product.price)}</div></div></button>)}</div>}</Shell>;
}
